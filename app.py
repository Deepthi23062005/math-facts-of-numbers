...
def load_math_facts():
    ...
    facts = {
        ...
        "text": [
            ...
            "A perfect number—equal to the exact sum of its proper positive divisors (1 + 2 + 4 + 7 + 14).",
            "The Answer to the Ultimate Question of Life, the Universe, and Everything, according to Douglas Adams.",
            "Roughly the inverse of the Fine-Structure Constant, which dictates the strength of the electromagnetic interaction."
        ]
    }
    ...
    if submit_button:
        ...
        with rc1:
            ...
            results_df = pd.DataFrame({
                ...
                "Calculated Output": [f"{tot_mass:.2f} M☉", f"{horizon:.2f} km", f"Alpha-State Balanced"]
            })
            st.table(results_df)
        ...
        with rc2:
            ...
            if (bh_mass / ns_mass) > 6.0:
                st.warning("⚠️ **Immediate Plunge:** The mass ratio is highly asymmetrical. The neutron star will likely cross the event horizon whole without leaving any significant accretion disk remnant.")
            else:
